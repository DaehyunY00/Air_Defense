...
## 현재 배포·분석 자료 (2026-09-21)

| 자료 | 범위 |
|---|---|
| [분석 방법과 결과](K-JAMDS_분석_방법과_결과.pdf) | SC3 전용. 생성된 모든 항적의 결과가 정해질 때까지 모의한 최종 결과, 20쌍 반복 비교, 조건별 비교, C2 기록과 항적 사례 |
| [지휘흐름 분석결과](K-JAMDS_지휘흐름_분석결과.pdf) | 위 문서를 간추린 13쪽판 — 결과 7개 장(조건별 설명 포함)과 분석 방법 3쪽을 분리. 위협 생성 강도 1배만, 시나리오 가정·모델 세부 가정 비교와 차이 없는 조건은 제외하고, 지휘 조건과 시간 가정을 나눠 실음. [소스](docs/analysis/2026-09-21-analysis-result/) |
...
이번 168개 비교 조건은 모두 미해결 0개이며, 생성 = 격추 + 누수가 성립합니다.

기준 비교(SC3 · seed 29 · 강도 1)는 각 실행에 같은 228개 항적을 사용했습니다.

| 배치 | 모드 | 격추 | 누수 | 미해결 |
|---|---|---:|---:|---:|
| LEGACY | As-Is | 97 | 131 | 0 |
| LEGACY | To-Be | 112 | 116 | 0 |
| FULL | As-Is | 186 | 42 | 0 |
| FULL | To-Be | 193 | 35 | 0 |

20개 시작값의 짝지은 비교에서 To-Be − As-Is 누수율 차이는
LEGACY 평균 −7.294%p(95% 신뢰구간 −8.260~−6.328),
FULL −4.310%p(−5.440~−3.180)입니다. 두 배치 모두 20쌍에서 To-Be의 누수가 적었습니다.
이 수치는 정해진 모델과 입력에서 얻은 최종 결과입니다.

이번 문서에서는 `timeout:c2`를 **발사 전 시간 종료(세부 원인 미분류)**로 풀어 씁니다.
특정 지휘소의 지연을 직접 입증한 분류로 읽지 않습니다.
`window_lost_due_to_c2`는 요격 가능한 시기를 놓친 것으로 분류한 경우이며,
`engagement_geometry_gap`은 책임 지휘부에 연결된 후보 범위의 기하 검사 결과입니다.

XLSX를 수정해도 시뮬레이터에 자동 반영되지 않습니다. PDF는 기록된 소스와 실행 설정에서
산출한 결과이며, 이전 날짜의 보고서·ADR 수치는 당시 조건의 기록으로 구분합니다.
...
2026-09-21 PDF와 최종 집계 자료를 사용하십시오. 생성 마감과 관측 종료 기준도 함께 확인하십시오.
