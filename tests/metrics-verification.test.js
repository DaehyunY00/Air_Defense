...
//   legacy MFR 확장 후 SC1은 x≥2.0, SC2는 x≥2.5에서 전환이 관측된다.
//   [2026-07 C2-VOICE-COORD-01 실험 변경] 음성협조 10~30s 균등분포에서 SC3 전환 시작이
//   x≥1.5 → x≥1.0으로 이동(협조 고속화로 후속 승인 부하 유입이 앞당겨짐).
//   구체 임계는 엔진 변경마다 이동하나(부하의 함수) 핵심은 불변: 저강도 0건 + 고강도에서 발생.
assert(SCENARIOS.every(function (id) {
  return UI_X.every(function (x) {
    return deleg[id][x].asis === 0 ||
      (id === 'sc1' && x >= 2.0) || (id === 'sc2' && x >= 2.5) || (id === 'sc3' && x >= 1.0);
  });
}), 'As-Is 분권 전환 발생 구간: SC1 x≥2.0 · SC2 x≥2.5 · SC3 x≥1.0 (확장 MFR 팬아웃 부하의 함수)');
