...
현재 기본 예시(SC3 · seed 29 · 600초 · 강도 1)의 전체 생성 수는 각 실행 89개입니다.

| 배치 | 모드 | 격추 | 확정 누수 | 종료 미해결 |
|---|---|---:|---:|---:|
| LEGACY | As-Is | 27 | 29 | 33 |
| LEGACY | To-Be | 29 | 28 | 32 |
| FULL | As-Is | 62 | 4 | 23 |
| FULL | To-Be | 62 | 5 | 22 |

이는 단일 seed의 재현 예시입니다. 구조 차이의 탐색은 PDF에 수록한 배치별 20쌍의 seed 차이와
95% 신뢰구간을 사용하며, 확정 누수 감소와 종료 미해결 증가를 구분합니다.
같은 seed의 To-Be − As-Is 누수율 차이는 LEGACY 평균 −1.784%p(95% CI −2.879~−0.689),
FULL −2.136%p(−4.006~−0.266)입니다. 분모는 각 실행의 생성 항적 수이며, 개념 모델의
해당 설정에서 얻은 탐색적 결과입니다. 누수가 증가한 seed도 각각 2개·5개 있습니다.
