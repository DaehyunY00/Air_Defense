# K-JAMDS C2 시뮬레이터 — KJADS 문제 상황 재현 + C2 모델 고도화(Phase 7)
...
| **SC3** | 전략적 섞어쏘기 | 전투기·무인기·TBM·방사포 **동시 복합 공격** — 방사포는 **파상(wave) 도착**으로 배치성 포화 재현, 처리용량 임계(ρ≥0.9) 초과 구간에서 As-Is↔To-Be 개선폭 정량화 |
| **SC4** | C2 통신 두절·분권 전환 (Phase 7) | 육↔공 협조 링크가 300~1,200초 **두절**(전자전·고장 개념) — 협조경로 상실 시 **권한위임(fallback) 분권 전환**(원칙 6 Degraded Mode)의 빈도·지연·복원력을 As-Is↔To-Be 비교 |

또한 SC1·SC2·SC3·SC4 전부에 **오경보(클러터) 트랙 스트림**(2022.12.26 "새떼·풍선 오인"
재현)이 흘러 식별·위협평가 서버 용량을 실위협과 함께 소모한다(격추/누수 통계에는 불산입).
...
js/
  ...
  core/
    ...
    rng.js                  # (Phase 2) Mulberry32 시드 RNG + 분포 샘플러 (지수·삼각·정규·로그정규·포아송)
    heap.js                 # (Phase 2) 이벤트 큐용 이진 최소힙
    hungarian.js            # ★ (Phase 7) 헝가리안(Kuhn–Munkres) O(n³) 최소비용 할당 — To-Be 배치 WTA
  analysis/
...
tests/
  ...
  phase7.test.js            # ★ (Phase 7) 헝가리안·우선순위 큐·오경보·재고·Degraded·파상도착
  analytic.test.js          # ★ (Phase 7) 해석해 교차검증 — M/M/c/K 이론값 vs DES 관측 (상설 회귀)
...
3. **노드 처리용량** — 유한용량 대기행렬 (서버 c, 처리시간 μ⁻¹, 용량 K; Phase 7부터 C2
   결심시간은 로그정규 — 검증 모드에서 M/M/c/K로 복원 가능) (`nodes.js`)
...
### DES 9단계 C2 파이프라인 (Phase 2 → Phase 7 고도화)
...
1 탐지 → 2 추적생성 → 3 식별 → 4 위협평가 → 5 WTA → 6 결심
   ...
   (오경보 트랙은 3·4단계 용량을 소모 후 기각/상위보고 — 격추/누수 통계 불산입)
```

- 각 C2·무기 노드는 유한용량 서버풀(서버 c개, 용량 K 초과 시 드롭=누수).
...
### Phase 7 — C2 모델 고도화 (선행연구 조사 반영, 2026-07)

공개 학술문헌·M&S 표준·해외 IAMD 공개자료 조사(로드맵 1·2단계)를 정량 모델에 반영했다.
방법론 근거와 신뢰도 등급은 `docs/params.md` "Phase 7" 절 참조 — **수치는 전부 정책연구용
개념값(C급)이며, 방법론(분포·큐 규율·알고리즘)의 출처 등급을 별도 표기**한다.

| # | 고도화 항목 | 구현 내용 | 파라미터 ID |
|---|---|---|---|
| 1 | **결심시간 로그정규화** | C2 노드 서비스시간을 지수→로그정규(CV=0.5)로 교체 — 인간 결심시간의 우편향 반영. `serviceDist:'exp'` 옵션으로 M/M/c 검증 모드 복원 | C2-SVC-DIST-01 |
| 2 | **비선점 우선순위 큐** | 위협 우선순위(1 탄도탄 → 4 소형무인기, 5 오경보) 순 처리, 동순위 FCFS. 포화 시 저우선 대기 폭증을 `priorityWait`로 관측 | THR-PRI-01 |
| 3 | **오경보(클러터) 처리 부하** | 시나리오별 오경보 스트림이 식별·위협평가 서버 용량 소모, 확률적 상위보고(As-Is 0.3/To-Be 0.1). 2022.12.26 "새떼 오인" 부하 재현 | ENV-FT-RATE/ESC-01 |
| 4 | **재고 임계치 관리 (원칙 5)** | 무기별 요격자산 재고 상태변수(발·소티 개념), 20% 예비율 이하 시 저우선 교전 보류·0이면 교전 불가(`inventory_denied`) | WPN-INV-01/RSV-01 |
| 5 | **Degraded Mode (원칙 6)** | 시나리오 `outages` 창 동안 링크 불가용 — 협조경로 상실 시 권한위임 지연(45초) 후 분권 자체승인(fallback). SC4 신설 | C2-OUTAGE-01, C2-FALLBACK-DLY-01 |
| 6 | **파상(비정상 포아송) 도착** | mix 항목 `wave`(ON/OFF 구간별 도착률)를 thinning으로 표본화 — SC3 방사포 연발의 배치성 포화(평균 λ는 종전과 동일) | ENV-WAVE-01 |
| 7 | **To-Be 배치 WTA (헝가리안)** | JAMDC2가 결심주기(4초)마다 대기 표적을 모아 [표적×무기채널] 비용행렬(1−Pk기대값·부하·명령지연)을 헝가리안 O(n³) 최적 할당 — Kill-Web/Any-Sensor-Best-Shooter의 알고리즘화. As-Is는 탐욕(최소부하) 유지 | WTA-HUNG-01, WTA-EPOCH-01 |
| 8 | **해석해 교차검증 상설화** | 검증 모드 DES를 M/M/c/K 이론값(ρ ±5%, Wq ±15%)과 자동 대조 + 로그정규의 M/G/c 방향성(Allen-Cunneen) 확인 — MIL-STD-3022 '결과 확인' 회귀 | tests/analytic.test.js |

결과 모달에는 "C2 프로세스 지표" 섹션(오경보 처리, 고/저우선 결심대기, 분권 전환 횟수,
요격자산 소모·고갈)이 추가되어 As-Is↔To-Be 비교와 함께 표시된다.
...
- [x] **Phase 7 — C2 모델 고도화 (선행연구 반영)**: 결심시간 로그정규화(CV=0.5)·비선점 우선순위 큐·오경보(클러터) 처리 부하·재고 임계치 관리(원칙 5)·Degraded Mode 권한위임(원칙 6, SC4 신설)·파상 도착(thinning)·To-Be 배치 WTA(헝가리안 O(n³))·M/M/c/K 해석해 교차검증 상설 회귀 — 방법론 근거는 `docs/params.md` Phase 7 절(공개 학술문헌·MIL-STD-3022·IBCS 공개자료 등 검증된 출처만 인용)

## 검증

```bash
# 전체 회귀 스위트 — 단일 진입점 (구문검증 + 7개 스위트 159 어서션, CI 게이트)
node tests/run-all.js

# 개별 실행
node tests/engine.test.js      # DES: 재현성·극한값·시나리오 병목·burst·보존·trace모드
node tests/phase7.test.js      # Phase 7: 헝가리안 최적성(브루트포스 대조)·우선순위 큐·오경보·재고·Degraded·wave
node tests/analytic.test.js    # 해석해 교차검증: M/M/c/K 이론값 vs DES 관측 (ρ ±5%, Wq ±15%)
