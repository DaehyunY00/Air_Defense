...
// ══════════ SC2: 협조가 사실상 전부 성립 ══════════
// [2026-07 C2-VOICE-COORD-01 실험 변경] 음성협조 10~30s 균등분포에서는 장체공(900s) 무인기의
// 잔여 체공창 대비 협조 지연이 충분히 짧아, 종전 "일부 책임공백·중복교전" 신호(gaps>0)가
// SC2에서 소멸한다(gaps=0). SC1·SC3의 gaps>0 검증은 위에서 유지된다.
console.log('# SC2 무인기 버스트 — 확장 ICC 음성협조 사실상 전부 성립');
var s2 = agg('sc2', 'asis', 2.5);
assert(s2.decon > 0 && s2.gaps === 0 && s2.dup === s2.gaps,
  'SC2 As-Is: 협조 성립(' + s2.decon + ')·책임공백(' + s2.gaps + ')=중복교전(' + s2.dup + ')=0 — 실험 변경 정본');
