...
- 5개 탭: **[시뮬레이션]**(지도·실행·결과창) · **[분석]**(같은 항적의 두 체계 소요시간 · **C2 결심시간 분해** · 항적 병렬 대조) · **[C2 구조]**(As-Is↔To-Be 계층도·항적 재생) · **[Monte Carlo]**(수렴·유의성·민감도·임계 전환점) · **[근거자료·제약검증]**(파라미터 근거·제약 어서션).
...
- **C2 결심시간은 ①만 주 지표입니다**(ADR-081). 「탐지 → 교전명령」 합계에는 C2가 아닌
  **사격통제 성립 대기**(포대 레이더가 표적을 물 때까지의 물리 대기)가 섞여 있어, To-Be가
  결심을 29초 앞당기면 그만큼 이 대기가 길어져 이득을 상쇄합니다. 합계만 인용하면 안 됩니다.
