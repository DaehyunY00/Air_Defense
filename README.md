...
- 링크 의미론은 codex(IADS_codex) 정본을 따릅니다(ADR-057): 센서→C2 보고주기 차등(GREEN_PINE 16s /
  ...
  `Normal(20, σ5)`. 세 값 모두 **등급 C**이고, 임무 지표는 유의하게 움직이지 않습니다
  (As-Is 자기쌍체 12 seed에서 7개 지표 전부 CI가 0 포함).
...
- **시간표는 한 장입니다**(ADR-083 — 종전 「결심시간 분해」·「구간별 소요시간」 두 표를 합쳤습니다).
  ...
  ⑤C2 출력로 나누며, 각 행의 **성격 배지**가 그 구간이 지휘구조의 몫인지, 지휘구조가 단축할 수
  없는 대기인지를 말합니다. ④를 뺀 잔여 정의라 **①+②+③+④+⑤ = 합계**가 항적마다 항등으로 성립합니다.
- **④ 「교전 가능 상태 대기」는 지휘구조가 단축할 수 있는 구간이 아닙니다.** 이 동안에도
  C2(WTA)는 후보 포대를 계속 평가하지만, 대기의 대부분(실측 89~95%)은 요격점 미성립·MFR
  사격통제라는 비행 기하·센서 물리이고 나머지는 탄약·동시교전 한도입니다. 상류(①②③)를
  앞당길수록 그만큼 길어져 합계에서 이득을 상쇄하므로 합계만 떼어 인용하면 안 됩니다.
  반대로 **③ 교전 협조는 사람 채널이 개입하는 유일 구간**이라 여기서 나는 Δ는 온전히
  지휘구조의 몫입니다(SC3 실측: As-Is 14.6초 → To-Be 0.7초).
...
- **적의 표적**은 축선당 1점이 아니라 **조준점 10점**입니다(ADR-097 `aim` — 이 화면만 기본 ON).
  ...
  효과: 착탄 격자 5→10, FULL 중·장거리 방공 12/39 → 21/39(천궁-II 2/22 → 9/22).
  > ⚠️ 누수는 늘어납니다(LEGACY 71→115 · FULL 16→40). 막을 수 없는 곳으로 위협이 가기
  > 때문이고, LEGACY는 조준점 10개 중 5개에 사거리 안 포대가 없습니다. 그리고 그것이 FULL의
  > 값어치를 드러냅니다 — FULL이 LEGACY보다 더 막는 양이 55→75건. 끄려면 `?aim=0`.
- **탄도탄 발사점**은 축선 연장선에서 시작합니다(ADR-091 `lx` — 이 화면만 기본 ON, 엔진 기본은
  OFF). 종전에는 탄도탄이 이미 모든 센서의 시야 안에서 생겨나 **그린파인 900km 조기경보가 한 번도
  먼저 보지 못했습니다**(srbm 최초 탐지 포대 전속 MFR 53/53). 켜면 그린파인 48/53으로 뒤집힙니다.
...
- 상단 **[LEGACY] / [FULL]** 스위치로 배치를 바꿉니다. LEGACY 64노드(포대 15)는 계선 흐름을
  ...
  > ⚠️ **두 배치의 수치를 섞어 인용하면 안 됩니다.** SC3·seed 12345·600초 실측으로 To-Be 우위가
  > LEGACY +8 vs FULL +15로 두 배 가까이 갈립니다. 그래서 배치 이름을 스위치와 실행 상태줄
  > 두 곳에 늘 적습니다. FULL은 계산이 무겁습니다(600초 5.6초 · 1800초 24초 · 메인 스레드).
...
| **`selfDefenseFire`** (+반경 `selfDefenseRadiusKm`) | **ON** | 자위권 사격 — 상급·원격 화력통제가 없어도 포대 10 km 이내 예측 탄착 탄도탄에는 자체 판단으로 사격(codex ADR-050 3단 사다리 ③). ⚠️ 교전 협조를 우회하므로 **As-Is 구제 효과가 더 커 To-Be 개선폭을 줄입니다** | ADR-071·072 |
| **`kvmfLateral`** | **ON** | 방공C2A 사이 **상급 경유 항적 공유**(KVMF 계통, 대표 30초·등급 C). 2022-12-26 공식 지적(1군단↔수방사 항적 공유 지연)이 걸리는 육군 계선이 모델에 없어, 두 방공C2A가 서로의 그림을 보려면 공군을 문자로 90초 왕복해야 했다. 양 모드에 존속하되 To-Be는 조율층 2초 경로가 이겨 선택되지 않는다. ⚠️ **임무 지표는 유의하게 움직이지 않는다**(30 seed 자기쌍체 8개 지표 전부 CI가 0 포함) — 채택 근거는 수치가 아니라 구조 정합. UI 토글·딥링크 없음 | ADR-081 |
...
- `sdf=0` 자위권 사격 해제(ADR-071·072). 끄면 상급·원격 화력통제가 없는 포대는 **자기 머리 위로
  ...
  자체 판단으로 사격합니다(codex ADR-050 3단 발사 사다리 ③). ⚠️ 이 경로는 교전 협조를 우회하므로
