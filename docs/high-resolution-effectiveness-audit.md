...
비교 우선순는 당시 읽기 전용 기준 commit `02eea4c`와 당시 legacy/OFF였다. 현재 `tests/phase0-baseline.json`은 legacy 10세트 확장 후 schemaVersion 2로 재기준선화되었으므로 이 문서의 과거 절대수치와 직접 비교하지 않는다. `IADS_codex_original`은 배치 및 목표 구현 경계의 정본으로만 사용했다.
...
- 스냅샷 시점 회귀: JS 25개 구문, 20개 스위트, 528개 어서션 전부 통과. 후속 구현 후 현행 기준은 JS 27개 구문·28개 스위트·664개 어서션이다.
- `git diff --check` 통과.
