...
//   legacy MFR 확장 후 SC1은 x≥2.0, SC2는 x≥2.5, SC3는 x≥1.5에서 전환이 관측된다.
//   구체 임계는 엔진 변경마다 이동하나(부하의 함수) 핵심은 불변: 저강도 0건 + 고강도에서 발생.
assert(SCENARIOS.every(function (id) {
  return UI_X.every(function (x) {
    return deleg[id][x].asis === 0 ||
      (id === 'sc1' && x >= 2.0) || (id === 'sc2' && x >= 2.5) || (id === 'sc3' && x >= 1.5);
  });
}), 'As-Is 분권 전환 발생 구간: SC1 x≥2.0 · SC2 x≥2.5 · SC3 x≥1.5 (확장 MFR 팬아웃 부하의 함수)');
