...
for(const dep of DEPLOYMENTS)for(const mode of ['asis','tobe']){
  ...
  assert.equal(b.decN,r.summary.timed,`${dep} ${mode}: timed sample differs`);
  assert.ok(Math.abs(b.dec-r.summary.total.mean)<1e-6,`${dep} ${mode}: engine mean vs lineage mean`);
...
const pages=[];
pages.push(`<div class="eyebrow">CURRENT SOURCE · SC3 통합 재실행 보고서</div><h1>K-JAMDS 지휘통제 모의분석<br>분석 방법과 결과</h1><p class="subtitle">SC3 복합 포화를 기준으로 실험 집계와 C2 기록(실제 통신 · 명령 계보 · 시간 분해)을 한 권으로 통합</p>
...
<h3>기준 실행: SC3 · seed 29 · 600초 · 강도 ×1</h3>
${tbl(['배치','구조','생성','격추','확정<br>누수','종료<br>미해결','중복<br>사건','발사<br>횟수','발사<br>항적','탐지→최초 발사<br>평균초'],crossRows.map(({dep,mode,b,r})=>[DEP[dep].split(' · ')[0],MOD[mode],b.spawned,b.killed,b.leaked,b.censored,b.dup,b.shots,r.summary.launched,f(r.summary.total.mean)]),'',[11,10,8,8,8,9,8,8,8,22])}
...
<p>탐지→최초 실제 발사 평균은 ${DEPLOYMENTS.map(dep=>`${depName(dep)} ${f(run(dep,'asis').summary.total.mean)}→${f(run(dep,'tobe').summary.total.mean)}초`).join(', ')}다. 양 모드의 발사 항적 집합이 달라 단독 인과 효과로 볼 수 없다. 기록이 잘린 실행은 ${truncatedRuns}/${runs.length}개이고, 최초 발사의 계보 연결은 ${timedAll}/${launchedAll}개 항적에서 가능했다.</p></div>
...
<p class="small"><b>통합 안내.</b> 종전의 별도 문서 「K-JAMDS C2 분석결과」를 이 문서에 합쳤다. 측정 정의는 2절, 과정·시간·통신·항적 카드는 7~11절이다. 두 문서에 따로 있던 요약·해석 한계·재현 절차·버전 표는 한 번만 싣는다. 본문은 SC3만 다루고 SC1·SC2는 3절의 참고 표로 남긴다.</p>`);
...
${tbl(['항목','기준 조건'],[['공통','SC3 · seed 29 · 600초 · ×1; 표에서 명시한 요소만 변경'],['기본 ON','협조계선, COP, 자위권, 탄도 발사점 연장, 표적 카탈로그, 결심 시간 동일화, 협조 파이프라인, ICC 중계 인가'],['기본 OFF','원격 교전, 적합도, 무인기 자체 교전, 조기경보 추가 보고, 처리시간 바닥, ECS 추가 집행시간, 별도 명령 발령 작업'],['공유·시간 옵션','연합 공유 null; ICC/ECS/발령 시간은 명시적 재정의 없음'],['코드 기본 ON','고해상도 배치, 표적 산포, 남부 축선, 링크 의미론, 보고주기 동일화, 신선도 모델']],'kv',[18,82])}
...
pages.push(`${h2(3,'SC3 seed 반복과 참고 기준선')}<h3>3-1. SC3의 20개 쌍대 seed: 차이 = To-Be−As-Is</h3>
...
<h3>3-2. 참고: SC1·SC2 기준선 · seed 29 · 600초 · ×1</h3>
${tbl(['배치','시나리오','구조','생성','격추','누수','미해결','누수/생성','중복','발사'],baselineRows.filter(r=>r[1]!==SC.sc3),'',[11,18,10,9,9,9,9,11,7,7])}
<p class="note">이 문서의 나머지 절은 모두 SC3다. SC1·SC2는 같은 설정으로 돌린 단일 seed 참고값이며 OAT·seed 반복·C2 기록을 수행하지 않았다. SC2는 위협 비행 시간이 길어 600초 종료 시 확정 결과가 적을 수 있고, 0% 누수를 방어 성공으로 해석하지 않는다. 중복은 엔진의 중복교전 사건 수이며 피해 위협 수나 단순 재발사 수와 같지 않다.</p>`);
for(const dep of deps)pages.push(`<h2>4. 조건 하나씩 바꾸기 · ${DEP[dep].split(' · ')[0]}</h2><p>SC3 · seed 29 · 600초 · ×1. 각 행은 기준값에서 해당 조건 하나만 바꾼다. Δ는 <b>변경 실행−같은 모드의 기준 실행</b>이다. 미해결 증감을 함께 제시한다.</p>
...
pages.push(`<h2>5. 부하 변화와 관측 대기</h2><p>SC3 · seed 29 · 600초. 노드의 전체 작업 평균 대기(Wq)와 관측 이용률(ρ)을 표시한다. 제대별 작업 종류가 다르므로 같은 수치도 역할을 확인해야 한다.</p>
...
<h3>6-2. 600초에 확정된 누수의 주원인 · LEGACY</h3>${tbl(['모델의 원인 분류','As-Is','To-Be','Δ'],reasonsRows(deps[0]),'',[58,14,14,14])}
<h3>6-3. 600초에 확정된 누수의 주원인 · FULL</h3>${tbl(['모델의 원인 분류','As-Is','To-Be','Δ'],reasonsRows(deps[1]),'',[58,14,14,14])}
...
<p>같은 표적에 대한 복수 발사는 발사 시각·사수·명령·격추 판정을 함께 추적해야 해석할 수 있다. OAT에서 차이가 작다는 이유만으로 중복의 원인을 구조 자체로 확정하지 않는다. 개별 항적의 기록은 11절 카드에서 확인한다.</p>
...
<div class="box notice"><b>두 경로로 같은 값을 얻었다.</b><p>엔진 값은 성공적으로 발사한 _onIadsFire 경계에서 수집한다. 계보 재구성은 기록된 발사에서 명령·결심·항적 접수를 거슬러 올라가 다시 계산한 값이며, 네 실행 모두 엔진 평균과 일치한다(생성기가 검사). 결과 필드 주석의 「최초 교전명령」은 실제 수집 위치와 다르다. 발사한 항적만의 평균이며 양 모드의 포함 항적이 다를 수 있다.</p></div>
...
<p class="note">설정 상한: traceCap 5,000 · flowTraceCap 500,000 · c2EventCap 500,000. 통신 사건 수에는 노드 접수/처리 관측도 들어가므로 통신 간선 도착 수와 다르다. 기록 절단과 600초 시점의 미해결(검열)은 별개다. JSON에는 항적별 시간 계보·정식 노드·관측 통신을 보존하고, 엔진 전체 raw 상태는 포함하지 않는다.</p>
<h3>13-3. 기계적 검산</h3><ul><li>모든 분석 셀에서 생성 = 격추 + 확정 누수 + 종료 미해결. 모든 실행의 주원인 합계 = 확정 누수.</li><li>20쌍의 각 seed에서 양 모드의 생성 위협 수 일치.</li><li>네 기준 실행의 생성·격추·누수·미해결·발사 횟수, 시간 측정 항적 수, 탐지→최초 발사 평균이 실험 집계와 C2 기록 사이에서 일치.</li><li>C2 파서 self-test: 다른 지휘 분기의 혼입, ICC 자식 명령의 계보, 다른 발사의 BDA 혼입, 기록 절단, 도착/실패/대기 상태와 정식 노드 필터를 합성 사례로 검증.</li><li>UI 파서의 실제 불리언 설정을 카탈로그에 전달하며 엔진·어댑터·프로토타입 SHA-256을 두 JSON에 저장. 표·본문 수치는 재실행 JSON에서 생성.</li></ul>
