...
> 📘 **처음 사용한다면**: [`docs/사용자_가이드.pdf`](docs/사용자_가이드.pdf) — 기초 조작부터
> 용어 사전, 화면별 설명, 시뮬레이션 결과 해석 방법(목적별 분석 레시피)까지 정리한 통합
> 가이드입니다(원본 `docs/사용자_가이드.html`, 재생성 `node scripts/build-guide-pdf.mjs`).
