...
(※ 위협 다양화 개편 — 중부축 추가·북측 진입점 — 이후 재기준선. 개편 전에는 SC1 ×3.0에서도
반전이 관측됐으나 새 믹스에서 SC1은 전 강도 개선으로 바뀌었고, 반전은 SC3 저강도에 남음):

| 시나리오 | 강도 | As-Is | To-Be | 방향 |
|---|---|---|---|---|
| SC1 | ×0.5~3.0 전 구간 | — | — | 항상 개선 (재기준선) |
| SC2 | ×0.5~3.0 전 구간 | — | — | 항상 개선 |
| SC3 | **×0.5** | 5.57배 | **9.85배** | **악화** |
| SC3 | **×1.0** | 7.91배 | **7.95배** | **악화** |
| SC3 | ×1.5~3.0 | — | — | 개선 |
...
## 6. 2026-07-27 전수 재검토 — 지표 계정 결함 7건 시정

§1의 18개 지표 매트릭스는 "계산·시각화·방향성"을 감사했으나, **계산 경로가 실행 모드(legacy /
native 고해상도)와 집계 범위(주 계통 / 중복교전 ghost)에 따라 갈리는 지점**은 다루지 않았다.
이번 재검토는 그 축을 대상으로 엔진·UI·MC·C2 리포트의 지표 산출식을 전수 대조했고, 결함 4건과
라벨·주석 문제 3건을 발견해 모두 시정했다. 회귀는 신설 스위트
**`tests/metrics-accounting.test.js`(어서션 21건)**로 고정했다.

이후 두 건이 추가로 드러났다 — **시정 7**은 §7의 As-Is↔To-Be 실험 중,
**시정 8·9·10**은 단일본 재생성에 맞춘 UI·시각화 전면 검토 중 발견했다.
셋 다 정적 대조로는 보이지 않고 **실제 값을 눈으로 읽어야** 드러나는 종류였다
(0.00으로 눌린 ρ, 100%를 넘는 비율, ❌로 뜬 어서션).
...
### 시정 7 (높음) — native C2 부하가 kind 분해에 노출되지 않아 ③④⑤ 병목이 0으로 표시

**발견 경위**: §7의 As-Is↔To-Be 전수 실험을 돌리던 중, MINI/FULL 셀의 "C2 항적처리 최대 ρ"가
전 시나리오에서 정확히 `0.00`으로 나오는 것이 이상해 원인을 추적하다 발견했다.

- **증상**: native 고해상도 경로는 항적처리 작업을 `kind='iads_track'`, ECS 명령수신을
  `'directive_reception'`으로 태깅하는데, `_results`의 kind 분해는
  `['track','approval','engage']` 3종만 노출한다. 그 결과 고해상도 실행에서
  **노드 전체 ρ는 0.908(병목 판정 수준)인데 분석 탭 ③④⑤ 카드의 ρ·Wq·드롭은 전부 0**으로 표시됐다.
  즉 고해상도 배치에서는 **C2 처리 병목 진단이 사실상 무력화**되어 있었다(시정 1과 같은
  "native 경로 미배선" 계열이나, 병목 진단 자체를 가리므로 영향이 더 크다).
- **시정**: 엔진은 `highResolutionDeployment`일 때만 kind 키에 `iads_track`·`directive_reception`을
  추가해 노출한다(legacy wire shape·Phase 0 SHA 보존). UI(`panels.js`)와 실험 집계는
  `'track'` 조회 시 `['track','iads_track']`을 함께 보도록 폴백을 둔다 — 두 경로는 배타적이라
  최대·합이 같다. 아울러 승인 ρ 툴팁에 "native는 별도 승인 홉 없이 책임 C2가 자체 승인하므로 0이며
  그 부하는 ③④⑤에 포함된다"를 명시했다.
- **검증**: MINI_NORMAL·SC3 ×1.0 seed 12345에서 ③④⑤ ρ `0.000 → 0.908`(노드 전체 ρ 0.908과 일치),
  iads-c2 프로파일도 `0.000 → 0.910`. legacy 결과의 kind 키는 종전 3종 그대로이며
  `baseline.test.js`의 전체결과 SHA-256이 불변임을 확인했다.

### 시정 8 (중간) — 협조 실패율이 119.56%로 표시 (비율의 분자가 이벤트 수)

**발견 경위**: 단일본 재생성에 맞춘 UI·시각화 전면 검토 중, Monte Carlo 탭의
"C2 MOP 쌍체 비교" 표에서 As-Is 협조 실패율이 **119.56%** 로 나오는 것을 확인했다.

- **증상**: `coordinationFailureRate = c2Command.coordinationFailures / spawned` 인데,
  분자 `COORDINATION_FAILED`는 **이벤트 수**다. 한 위협이 중복교전 조율 실패·재시도마다
  이벤트를 여러 번 낼 수 있어(`_deconflict`의 책임공백, native `fire_commit`의
  `stale_or_unshared_engagement_state`) 비율이 1을 넘는다. `responsibilityUnresolved`도
  IADS 재시도 루프 안에서 반복 발생해 같은 성질을 갖는다. 라벨은 "(생성 대비)"로
  비율임을 명시하고 있어, 100%를 넘는 값은 독자에게 그대로 오독된다.
- **시정**: 이벤트 수는 진단용으로 그대로 두고, 비율의 분자를 **한 번이라도 겪은 위협 수**로
  분리했다(`coordinationFailedThreats`·`responsibilityUnresolvedThreats`, `threatId` 기준 중복 제거).
  분자 ≤ `spawned`가 보장되므로 값이 [0, 1]에 든다. 라벨도
  "협조 실패 **위협** 비율(생성 대비)"로 바꿔 분자가 위협임을 드러냈다.
- **검증**: SC1 ×1.0 seed 12345 · 30 쌍대복제에서 As-Is `119.56% → 21.84%`
  (To-Be 0.00%, Δ 95% CI [-24.41%, -19.26%] — 0 제외 판정은 종전과 동일).
  `c2-analysis.test.js`에 같은 위협의 반복 협조 실패가 위협 1건으로 계수되는지
  어서션을 추가했다.

### 시정 9 (중간) — 제약 B의 USFK 술어가 정의와 반대로 적혀 상시 실패

**발견 경위**: 같은 UI 검토에서 [근거자료·제약검증] 탭의 5개 어서션 중
**B가 ❌ 로 표시**되는 것을 확인했다.

- **증상**: `KJ.runConstraintChecks`의 제약 B는 FULL 배치의 USFK 사수가
  `canEngage`를 **전부 false**로 가질 것을 요구했다. 그러나 "독립축"의 정의는
  *한국군과 C2 링크로 이어지지 않고 한국군 WTA 후보에 오르지 않는다*는 것이지,
  USFK 사수가 자기 축에서 교전하지 않는다는 뜻이 아니다.
  `tests/deployment-adapter.test.js`는 정반대로 **`canEngage`에 true가 하나 이상 있고
  `c2Axis`가 `USFK_`로 시작할 것**을 어서트하고 있었다 — 두 정의가 충돌한 채
  화면 쪽만 상시 실패하고 있었다.
- **원인**: 어서션 함수 `KJ.runConstraintChecks` 자체를 실행하는 테스트가 없었다.
  `constraints.test.js`는 같은 제약을 **데이터 수준으로 다시 구현해** 검사했기 때문에,
  화면이 호출하는 함수의 술어가 틀려도 회귀 스위트가 통과했다.
- **시정**: 제약 B의 세 번째 절을 어댑터 테스트와 같은 정의(`USFK_` 축 소속 + 교전 가능)로
  교체하고 상세문을 "KAMDOC 교차 링크 없이 USFK_ 축에서만 교전 — 한국군 WTA 후보 아님"으로
  다시 썼다. 아울러 `constraints.test.js`에 **(g) `KJ.runConstraintChecks()` 전 항목 통과**
  절을 신설해, 화면이 실제로 부르는 함수를 최소 DOM 스텁 위에서 직접 실행하도록 했다.
- **검증**: 모듈판·단일본 양쪽에서 A~E 5건 전부 ✅. 배치 카탈로그 로드가 legacy `KJ.NODES`를
  오염시키지 않는다는 어서션도 함께 걸었다.

### 시정 10 (낮음) — 반올림으로 두 값이 같게 보이는데 Δ는 "개선"

분석 탭 ① 탐지율이 As-Is `98%` · To-Be `98%` 인데 판정은 `▲ 1%p 개선`으로 적혔다.
`fmtVal`이 비율을 `toFixed(0)`으로 찍어 97.95%와 98.47%가 둘 다 "98%"가 된 탓이다.
값 행과 Δ 행이 서로 모순돼 보이면 독자는 Δ 쪽을 의심하게 된다.

- **시정**: 기본 자릿수로 두 값이 같게 **보일 때만** 한 자리를 더 준다
  (`98.0% → 98.5% ▲ 0.5%p 개선`). 한 자리를 더 줘도 같으면 그 차이는 표시 해상도 아래이므로
  Δ를 "동일"로 접는다 — 없는 정밀도를 지어내지 않는다. 초·비율·배수 전 종류에 동일 규칙.
- **검증**: SC3 ×1.5 legacy 분석 탭 31개 지표행에서 "값은 같은데 개선/악화"가 0건.
