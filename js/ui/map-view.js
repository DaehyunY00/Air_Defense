...
(function () {
  ...
  function catalogFor(state) {
    ...
    // ADR-065: 승인 계선·남부 축선 토글이 카탈로그 구성(coord 링크·coverage)을 바꾸므로
    // 지도도 실행과 같은 변형을 그려야 한다. 종전에는 토글을 반영하지 않아, 사용자가 끈
    // 상태에서도 지도에는 승인 협조 링크가 남았다.
    return KJ.resolveModelCatalog({
      ...
      features: {
        ...
        sensorReportParity: !state || state.rp !== '0' // ADR-067
      }
    });
  }
