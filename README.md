...
지도에서는 동일 사이트의 ICC·ECS·MFR·포대를 저배율에서 픽셀 단위로 방사형 분리하고
실제 좌표까지 리더선을 표시합니다. zoom 11 이상에서는 실제 좌표로 복귀합니다. 범례 상단의
`legacy 확장 배치 · 활성 63/64노드 · 10세트 포함` 문구로 수정본 로딩 여부를 확인할 수 있으며,
과밀 방지를 위해 C2 연결선은 기본 OFF입니다. Leaflet이 없는 SVG fallback에서도 동일한
자산 분리와 탐지·교전 범위 링 토글을 지원합니다.
...
tests/  run-all.js + 28개 스위트  # 아래 [검증] 참조
```
...
```bash
node tests/run-all.js            # 전체 회귀 — JS 27개 구문검증 + 28개 스위트·664 어서션
```
...
| `map-visualization` · `ui-performance` | Leaflet/SVG 40개 자산·범위 링·저배율 분리 / Worker·지도 토글·범례 |
...
결과 모달은 `격추`, `확정 누출`, `관측 종료 미해결`을 전체 생성 위협 기준으로 따로 표시합니다. `global.killRate/leakRate`는 종료 시점 미해결을 제외한 **해결분 기준** 파생지표이므로, 전체 생성 기준 비율과 구분해야 합니다. native 고해상도 경로는 무한 재교전을 막기 위해 표적당 전 책임 C2 축 합산 최대 2발을 발사합니다. 현 PSSEK·PIP·센서 상태는 과도기 개념 모델이므로 FULL/MINI 절대값은 전술 성능치로 사용하지 마십시오.
