...
- **시간표는 한 장입니다**(ADR-083 — 종전 「결심시간 분해」·「구간별 소요시간」 두 표를 합쳤습니다).
  탐지를 0초로 놓고 발사까지를 ①C2 입력 → ②C2 처리 → ③**승인·협조** → ④사격통제 성립 대기 →
  ⑤C2 출력로 나누며, 각 행의 **성격 배지**가 그 구간이 지휘구조의 몫인지 물리의 몫인지를 말합니다.
  ④를 뺀 잔여 정의라 **①+②+③+④+⑤ = 합계**가 항적마다 항등으로 성립합니다.
- **④ 「사격통제 성립 대기」는 C2 성능이 아닙니다.** 포대 레이더가 표적을 물고 요격점이 설
  때까지의 물리 대기이며, 상류(①②③)를 앞당길수록 그만큼 길어져 합계에서 이득을 상쇄합니다.
  합계만 떼어 인용하면 안 됩니다. 반대로 **③ 승인·협조는 사람 채널이 개입하는 유일 구간**이라
  여기서 나는 Δ는 온전히 지휘구조의 몫입니다(SC3 실측: As-Is 14.6초 → To-Be 0.7초).
