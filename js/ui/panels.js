...
(function () {
  ...
  function timelineBar(r, side) {
    ...
    if (r.approvalJobs > 0 && decide != null) {
      marks += '<div class="dc-appr" style="left:' + decide.toFixed(2) +
        '%" title="교전 협조 절차 ' + r.approvalJobs + '건">◆</div>';
    }
    if (decide != null) {
  ...
  var PIPE = [
    ...
    { key: 'appr', approval: true, reach: null,
      label: '③ 교전 협조', tag: 'C2 구조', badge: 'bd-struct', primary: true,
      why: 'As-Is에서 사람 채널(국지방공 AOC↔MCRC 음성 교전 협조 — 상하 관계 아님, ADR-098)이 개입하는 유일 구간. 협조 불요 축·권한위임은 0초' },
    { key: 'fc', residual: true, from: 'prep', to: 'assign', reach: 'assign',
      label: '④ 교전 가능 상태 대기', tag: '대기 요인', badge: 'bd-phys',
      why: '쏠 수 있는 포대가 생길 때까지 — 이 동안에도 WTA는 후보를 계속 평가한다. 요격점 미성립·MFR 사격통제가 대부분이고, 탄약·동시교전 한도와 교전중복해소 대기가 섞인다' },
  ...
  KJ.panels = {
    ...
    renderAnalysis: function (state) {
      ...
      el('analysis-pipeline').innerHTML = !total.n
        ...
        '<b>④는 지휘구조가 단축할 수 있는 구간이 아닙니다</b> — 이 동안에도 C2(WTA)는 후보 포대를 ' +
        '계속 평가하지만, 대기의 대부분(실측 89~95%)은 요격점 미성립·MFR 사격통제라는 비행 기하·센서 ' +
        '물리이고 나머지는 탄약·동시교전 한도(무기체계 자원)입니다. ' +
        '그래서 ④가 늘어난 것을 "To-Be가 느려졌다"로 읽으면 안 됩니다.<br>' +
        '<b>③ 교전 협조</b>는 <b>사람이 개입하는 유일한 구간</b>입니다 — 마크가 없는 항적(협조 불요 축 ' +
        'KAMDOC·MCRC, 동적 권한위임)은 제외가 아니라 <b>0초</b>로 셉니다. 코호트 ' + total.n +
        '건 중 실제로 협조 계선을 경유한 항적은 <b>As-Is ' + rows.approvedN.a + '건 → To-Be ' +
        rows.approvedN.b + '건</b>입니다.<br>' +
        '⚠️ <b>④에는 교전 협조 외 조율 대기도 섞여 있습니다</b> — 교전중복해소(MCRC·COP) 대기가 그것이며, ' +
        ...
        '「여기까지 온 항적」 열과 아래 ③ 항적 로그가 담당합니다. ' +
        '시간표는 각 관문의 <b>최초 시각</b>만 쓰므로 <b>첫 발사까지</b>만 잽니다 — BDA 후 재교전은 ' +
        '협조 재취득 없이 ④로 복귀하며(협조 상태는 항적·지휘관별로 유지됨), 2발째 이후는 시간표가 ' +
        '아니라 격추·요격탄 소모 등 임무 지표가 담당합니다.<br>' +
