...
console.log('\n== 6) UI 계약 — 카드 2장이 1장으로 ==');
{
  ...
  // (표현 교정: "C2 성능이 아니다"는 과했다 — ④ 동안에도 WTA는 후보를 계속 평가하고,
  //  후보 집합은 C2 구조가 정한다. 잠그는 명제는 「지휘구조가 단축할 수 있는 구간이 아니다」다.)
  assert(/지휘구조가 단축할 수 있는 구간이 아/.test(panels),
    '④가 지휘구조로 단축되는 구간이 아님을 명시');
  assert(/교전중복해소/.test(panels), '④에 교전중복해소 대기가 섞임을 공시');
  assert(/격추·누수를 대체하지 않습니다/.test(panels), '임무 지표 대체 금지 문구 유지');
  // 재교전은 ③으로 돌아가지 않는다(협조 상태 유지) — 시간표가 첫 발사까지만 잼도 공시.
  // ADR-098: 화면 표현은 「승인」→「협조」(군단 AOC↔MCRC는 협조 관계).
  assert(/첫 발사까지/.test(panels) && /협조 재취득 없이 ④로 복귀/.test(panels),
    '재교전 경로(④ 복귀·협조 유지)와 시간표 범위(첫 발사)를 공시');
  // 성격 배지 5종 — Δ가 어느 성격의 구간에서 났는지가 표에서 바로 읽혀야 한다.
  ['C2 입력', 'C2 처리', 'C2 구조', '대기 요인', 'C2 출력', '센서 물리'].forEach(function (tag) {
