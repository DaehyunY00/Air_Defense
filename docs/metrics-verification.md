...
## 5. 질문 (임의 구현 금지 — 사용자 결정 필요)

1. **Lq(대기열 길이)를 노드표에 열로 추가할까요?** (DES 결과표 `sim-view.js:604-621`, 해석 탭
   `panels.js:44-60` 양쪽 모두 해당). Wq로 대체 가능한 정보라 우선순위는 낮습니다.
2. **분권 전환(delegation)을 vsCompare 비교 행으로 추가할까요?** 추가하더라도 현재 임계값·
   공식 시나리오 하에서는 To-Be가 항상 0건으로 나와 비교의 의미가 제한적입니다. 만약 이 지표를
   실제로 관측 가능하게 만들고 싶다면 `DELEG_QUEUE_MULT.tobe`(현재 1)를 낮추거나 공식
   시나리오의 부하 프로파일을 조정하는 **모델 파라미터 변경**이 필요한데, 이는 회귀 스냅샷·
   `docs/params.md` 근거값에 영향을 미치는 결정이라 임의로 진행하지 않았습니다.
3. **비용교환비 vsCompare 행에 "항상 개선되는 지표가 아님"이라는 안내를 추가할까요?** (툴팁
   문구 수정만으로 가능, 계산 로직 변경 없음)
4. **원인 taxonomy 8종 참조표를 근거자료 탭에 추가할까요?** 최저 우선순위 항목입니다.
