...
(function () {
  ...
  function Simulation(cfg) {
    ...
    // ADR-095: ECS 교전명령 실행 시간을 ADSIM과 맞춘다. 우리 ECS는 노드 카탈로그 값(체계 1~2초 +
    // 운용자 2초 = 3.5초)을 쓰는데, ADSIM은 역할 기반으로 `executor` = localExecution **10초**를
    // 쓴다(같은 자리·같은 일에 3배 차이). ON이면 directive_reception 서비스를 10초로 둔다.
    // ⚠️ **양 모드 공통**이다 — As-Is·To-Be 사슬에 같은 크기로 더해지므로 아키텍처 비교(Δ)는
    //    구조상 거의 불변이고, 움직이는 것은 절대 시간선과 그로 인한 교전창 상실이다(§수치 영향).
    // 기본 OFF(불변 규칙 1 · 재기준선 대상). 손잡이 ecsExecutionSec(기본 10초).
    this.ecsExecutionTime = ff('ecsExecutionTime', false);
    this.ecsExecutionSec = (typeof f.ecsExecutionSec === 'number' && f.ecsExecutionSec >= 0) ? f.ecsExecutionSec : 10;
    // ADR-096: 교전명령 발령 — 결심 노드가 명령을 **낼 때마다** 큐 작업을 태운다.
    // 종전에는 「위협판단·표적할당 준비」서비스 1회가 항적당 유일한 비용이었고, 실제 발령은
    // 그 콜백 안에서 0초였다. 실측(SC3 As-Is): KAMDOC 명령 58건 중 **69%(40건)가 준비 완료보다
    // 나중**에 나갔고(중앙값 +22.8초·최대 +113초) 그 40건은 큐를 다시 타지 않았다.
    // 창이 열리길 기다리다 내는 발령, ICC가 반송한 뒤의 재발령, 같은 항적의 두 번째 명령이
    // 전부 무료였다 — 특히 **반송 후 재결심 0초**는 어떤 해석으로도 방어되지 않는다.
    // 서비스 시간은 새로 짓지 않고 **그 노드의 체계 성분 중점**(ADR-092 serviceParts.systemSec)을
    // 쓴다: 운용자 판단은 준비 단계에서 이미 지불했고 발령은 체계가 하는 일(명령 생성·전송)이다.
    // KAMDOC·MCRC 7.5초 · ICC 4초 · IAOC·국지방공 1.5초 — ADSIM의 relay 10초·executor 10초와
    // 같은 크기대다(ADSIM은 발령을 결심에 포함시켜 별도 비용을 두지 않으므로 직접 방증은 없다).
    // ⚠️ ADR-078 §미구현이 무너뜨린 것은 **노드 평균 37.5초**를 중계에 오적용한 경우다. 여기서는
    //    kind별 평균(ADR-095 기구)으로 체계 성분만 쓰므로 그 함정이 아니다 — 그래도 ρ가 오르므로
    //    손잡이 directiveIssueSec와 민감도를 함께 둔다. 기본 OFF(불변 규칙 1 · 재기준선 대상).
    this.directiveIssueTime = ff('directiveIssueTime', false);
    this.directiveIssueSec = (typeof f.directiveIssueSec === 'number' && f.directiveIssueSec >= 0) ? f.directiveIssueSec : null;
  ...
  /**
   * ADR-094·095: 노드의 kind별 서비스 시간 표. 손잡이가 켜진 kind만 들어간다.
   *  - directive_relay      ICC 중계 인가 (ADR-094 · iccRelaySec)
   *  - directive_reception  ECS 교전명령 실행 (ADR-095 · ecsExecutionSec, ADSIM localExecution 10초)
   * ⚠️ 노드 기본값과 같은 값을 굳이 넣지 않는다 — 비면 `_startService`가 분기 자체를 타지 않아
   *    OFF 경로가 자구 동일하게 유지된다.
   */
  function kindMeanOf(n, sim) {
    if (n.category !== 'c2') return null;
    var m = null;
    if (sim.iccRelaySec !== null) (m = m || {}).directive_relay = sim.iccRelaySec * sim.mult.service;
    if (sim.ecsExecutionTime) (m = m || {}).directive_reception = sim.ecsExecutionSec * sim.mult.service;
    if (sim.directiveIssueTime) {
      var sp = n.queue && n.queue.serviceParts;
      var v = (sim.directiveIssueSec !== null) ? sim.directiveIssueSec
        : (sp ? (sp.systemSec[0] + sp.systemSec[1]) / 2 : null);
      if (v !== null) (m = m || {}).directive_issue = v * sim.mult.service;
    }
    return m;
  }
  ...
  Simulation.prototype._initNodes = function () {
    ...
    this._nodesInMode().forEach(function (n) {
      ...
      self.nodeState[n.id] = {
        ...
        // ADR-094·095: **kind별 서비스 시간**(ADR-078 §미구현이 요구한 기구의 최소 형태).
        // 비어 있으면 노드 기본값을 그대로 쓴다 — 그 경우 OFF 경로와 자구 동일하다.
        kindMean: kindMeanOf(n, self),
  ...
  Simulation.prototype._startService = function (ns, t, job, onDone) {
    ...
    // ADR-094·095: kind 전용 평균이 있으면 그것을 쓴다(ADR-078 §미구현이 요구한 kind별 서비스시간의
    // 최소 형태). 표가 비어 있으면 nsMean·nsParts는 노드 기본값 그대로라 OFF 경로는 불변이다.
    // ADR-092의 바닥은 성분을 같은 비율로 스케일해 합성한다 — 두 플래그가 겹쳐도 뜻이 유지된다.
    var nsMean = ns.mean, nsParts = ns.parts;
    var kindMean = ns.kindMean ? ns.kindMean[job.kind] : undefined;
    if (kindMean !== null && kindMean !== undefined) {
      var kScale = ns.mean > 0 ? kindMean / ns.mean : 0;
      nsMean = kindMean;
  ...
  Simulation.prototype._iadsDecide = function (threat, t, commander) {
    ...
    var dd = { threat: threat, commander: commander, shooterId: chosen.shooter.id, plan: plan, path: path };
    // ADR-096: 발령도 일이다 — 결심 노드 큐를 거쳐야 명령이 계선을 탄다.
    if (this.directiveIssueTime && this.nodeState[commander.id]) this._iadsIssueDirective(t, dd);
    else this._iadsDispatchDirective(t, dd);
  };

  /**
   * ADR-096: 교전명령 발령 작업(`directive_issue`) — 결심 노드가 명령을 낼 때마다 태운다.
   * 첫 발령이든 ICC 반송 뒤의 재발령이든 같다. 서비스가 끝나야 하달이 시작된다.
   */
  Simulation.prototype._iadsIssueDirective = function (t, d) {
    var self = this, threat = d.threat, plan = d.plan;
    var disp = this._nodeArrive(d.commander.id, t, {
      threat: threat, kind: 'directive_issue', commander: d.commander,
      jobId: 'ISSUE_' + plan.directiveId, directiveId: plan.directiveId, validUntil: plan.validUntil,
      onAbandon: function (at) {
        if (plan.released || plan.resolved) return;
        self._iadsTransitionPlan(plan, 'expired', at, 'released', 'issue_queue_deadline');
        self.global.c2Orders.expired++;
        self.global.c2Orders.released++;
        self.global.c2Orders.expiryByReason.issue_queue_deadline =
      ...
      self._mark(threat, '교전명령발령:' + d.commander.id + '→' + d.shooterId, done, axisOf(d.commander));
      self._iadsDispatchDirective(done, d);
    });
    if (disp === 'dropped') {
      this._iadsTransitionPlan(plan, 'expired', t, 'released', 'issue_queue_capacity');
      this.global.c2Orders.expired++;
      this.global.c2Orders.released++;
      this.global.c2Orders.expiryByReason.issue_queue_capacity =
  ...
  /** 교전명령 하달 — ADR-094의 ICC 중계 분기를 포함한 전송 개시. */
  Simulation.prototype._iadsDispatchDirective = function (t, d) {
    var path = d.path, plan = d.plan;
    var relayAt = this.iccRelayAuthorization
      ? this._iadsRelayIccIndex(path, d.commander.id) : -1;
    this._iadsTransitionPlan(plan, 'in_transit', t);
    if (relayAt >= 0) {
      var d1 = this._iadsSendAlong(path, 0, relayAt + 1, t, d.threat);
      this.schedule(t + d1, PRI.LINK_ARRIVE, 'IADS_ICC_RELAY', {
        threat: d.threat, commander: d.commander, shooterId: d.shooterId, plan: plan,
        iccId: path[relayAt].to, path: path, from: relayAt + 1
      });
      return;
    }
    var delay = this._iadsSendAlong(path, 0, path.length, t, d.threat);
    ...
    if (this.trace) {
      if (this.threatTraces.length < this.traceCap) {
        threat._trace = {
          ...
          // ADR-064·091이 축선·발사점 연장으로 조정한 **최종** 체공창. 소비처는 위치 재구성이다:
          // 시각 t의 위협 위치 = KJ.axisPosition(axis, (t - spawnT) / dwellSec, target).
          // ⚠️ 관측 전용이다 — 이 값은 이미 확정된 threat.dwellSec의 사본이고 동역학에 되먹이지
          //    않는다. 화면이 이것을 재계산하면 안 되는 이유: 분모는 KJ.axisDwellSec에 더해
          //    ballisticLaunchAxes의 lx.scale과 **엔진 내부 판정**(iadsThreatCategory —
          //    공개 대응물이 없다)까지 걸린 값이라, 밖에서 다시 짜 맞추면 그 플래그에서
          //    조용히 어긋난다(실측: lx ON에서 uav_small·fighter 17건 불일치).
          dwellSec: threat.dwellSec,
          // ADR-091 발사점 연장량(km). 0이면 축선 진입점에서 시작한다는 뜻이다.
          // ⚠️ dwellSec과 한 짝이다 — 이 둘이 있어야 화면이 KJ.iadsThreatPosition으로
          //    궤적을 되짚을 수 있다. 관측 전용이고 동역학에 되먹이지 않는다.
          launchExtKm: threat._launchExtKm || 0,
  ...
  Simulation.prototype._results = function () {
    ...
    var nodes = Object.keys(this.nodeState).map(function (id) {
      ...
      if (self.directiveIssueTime) kindKeys = kindKeys.concat(['directive_issue']);   // ADR-096
  ...
  // 위협 위치 재구성용 공개 helper — 순수 읽기(RNG·상태 미변경). 화면(프로토타입 [지휘 흐름]
  // 지도)이 궤적을 되짚을 때 **엔진과 같은 함수**를 쓰게 하려는 것이다. 밖에서 다시 짜 맞추면
  // ADR-091 발사점 연장(재매개변수화 p' = p·f − (f−1))에서 조용히 어긋난다.
