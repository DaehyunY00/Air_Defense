...
3. 탄약·재장전·BDA
   ...
   - 명중은 발사 시 시드 RNG로 결정하고 요격체 PIP 도달 시 BDA를 처리한다. MISS는 잔여 교전창 내에서 1회 재교전하며, 표적당 전 C2 축 합산 최대 2발으로 제한한다.
4. 중복교전
...
- 당시 전체: JS 27개 `node --check`, 26개 스위트·595개 어서션. legacy 지도 보완 후 현행은 28개 스위트·664개 어서션.
...
## 2026-07-20 요격 실패 0% 교정

- 원인 1: native 고해상도 경로가 모든 미사일에 공통 `PSSEK.default=0.75`를 적용해 비호·천마의 이식된 호환 Pk 0.30을 무시했다.
- 원인 2: MISS 후 체공창이 닫힐 때까지 발사 횟수 상한 없이 재교전해 누적 격추확률을 사실상 1로 만들었다.
- 원인 3: `censorFix` 지표는 관측 종료 미해결을 분모에서 제외하지만, UI가 이를 보여주지 않아 `45 격추 + 0 누출 + 5 미해결`을 100% 격추로 읽게 했다.
- 교정: SHORAD 과도기 Pk 0.30 적용, 표적당 shoot-look-shoot 2발 상한, 전체 생성 기준 격추/확정 누출·관측 종료 미해결·해결분 기준 비율 분리 표시.
- 재현: `HANBANDO_FULL_NORMAL`, SC2, As-Is, 1.0×, seed 12345, 1800초에서 `37 생성 / 19 격추 / 12 확정 누출 / 6 미해결`. 전체 생성 기준 확정 누출률 32.4%, 해결분 기준 38.7%다.
- 아닌 원인: NORMAL에서 ECS/ICC가 상위 명령 타임아웃 후 자율 발사하는 전이는 없다. 사례 시나리오의 UAV는 초기부터 `ARMY_LOCAL_AD(self_battery)` 독립 책임 축으로 분류된다. 명령 경로가 없으면 `responsibility_gap`으로 누출되며 자율 발사하지 않는다.
- 한계: 원본의 위협·거리·aspect별 PSSEK 테이블과 센서 손실/항적 신선도는 아직 미이식이다. 이번 0.30은 현 과도기 adapter의 `compatibility.pk`와 일치시킨 값으로, 전술적 실제 Pk로 해석하지 않는다.
- 회귀: `tests/iads-failure-realism.test.js` 8건이 Pk, 보존법칙, 명중 실패 누출, 2발 상한, ECS/ICC 타임아웃 비존재를 고정한다.
