...
정적 웹 페이지이므로 별도 빌드가 필요 없습니다. 두 가지 방법이 있습니다.
...
| **요격체계 세분화(WP1)** | 집계 shooter를 ICC(대대)→포대(ECS+MFR+TEL[]) 계층으로 대체·발사대 재고·재장전·티어 핸드오버 — 병목이 C2→화력단위로 이동(ADR-010) | `fireUnitLayer`(OFF) |
| **자체교전(WP2)** | 포대 MFR이 상위 C2 명령 없이 자체 탐지·자율 요격(자위권). 양 모드 공통 → As-Is 하한↑·To-Be 개선폭↓(반증 성격, ADR-011) | `selfDefense`(OFF) |

> **최대 충실도 실행**: `KJ.PRESETS.highFidelity`(= fireUnitLayer·selfDefense·magazine·reserveFloor ON). 기본 실행은 두 신규 플래그 모두 OFF(legacy 지문 비트 동일).
...
js/
  data/  nodes.js · links.js · threats.js · scenarios.js · axes.js · fire-units.js
         # 노드(대기행렬·pk·wtaSuit·costPerShotM·magazine)·모드별 링크·위협·시나리오·축선 좌표
         # fire-units.js: ICC(대대)→포대(ECS+MFR+TEL) 세분화 계층(WP1, features.fireUnitLayer)
  core/  router.js · constraints.js · rng.js · heap.js
         # 딥링크 라우터 · 제약 어서션 · Mulberry32 RNG+샘플러 · 이벤트 큐 min-heap
  analysis/  bottleneck.js · mc-runner.js · overlap-heatmap.js · transition.js
         # 해석적 병목 근사 · Monte Carlo · 중복교전 히트맵 · 임계 전환점
  engine/  sim-engine.js         # ★ DES 엔진: 9단계 파이프라인·M/M/c/K·기능 플래그·병목 도출·leak taxonomy
...
tests/  run-all.js + 16개 스위트  # 아래 [검증] 참조
```
...
```bash
node tests/run-all.js            # 전체 회귀 — 구문검증 + 16개 수치 스위트 398 어서션 (CI 게이트)
```
...
제약조건 어서션(신궁·천마 탄도탄 교전 불가, THAAD 미모델링, 디스클레이머 상시 표출, 개념좌표 주석, KF-21 보라매 표기)은 **[근거자료·제약검증] 탭**에서 상시 확인됩니다.
