...
- **연결**: MFR→ECS, ECS↔ICC, ECS→포대. 상향 협조는 계선별 분리(ADR-053, 2026-07-27
  사실 정정): 천마(육군) 세트는 ICC↔AOC-1C KVMF 30초(ADC2A 계선, MCRC 직접 링크 없음 —
  육↔공 협조는 AOC-1C↔MCRC 음성으로 수렴), 천궁-II(공군 방공유도탄사) 세트는 ICC↔MCRC
  Link-16 12초 데이터링크. To-Be는 MFR·ICC↔JAMDC2 및 상향 링크 전부 데이터링크 2초.
  (종전 "전 세트 ICC↔MCRC 음성/VTC 직결"은 공개자료 근거 부재로 폐기 — 근거: 천궁 블록II
  작전통제소 Link-16 연동·공군 방공유도탄사 소속(등급 B), ADC2A/KVMF 육군 계선(등급 B),
  2022-12-26 1군단→공작사 유선전화 실증(등급 B). 상세 출처는 ADR-053.)
...
> **[C2-VOICE-DLY-01] 폐기·분해됨 (feat/stage2-track-overhaul Phase 2).** 하나의 paramRef가 3개 값(음성 180s·KVMF 30s·데이터링크 2s)을 정당화하고, 링크 1단계 지연으로 오적용되던 문제를 아래 5개로 분해한다. 원 출처의 "3분 이상"은 **경로 총합(end-to-end)**임을 `C2-RESP-E2E-01`에서 명시한다.

### [C2-RESP-E2E-01] 작전반응시간 (탐지→대응, end-to-end) — 경로 총합 검증용
- **값/분포**: As-Is ≥180 s (원 출처 "3분 이상") → To-Be ~30 s. **링크 1단계 지연이 아니라 경로 총합 상한**으로만 사용.
- **단위**: 초 (경로 총합)
...
- **신뢰도 등급**: B
- **MC 적용방식**: 검증 상한(고정)
- **재해석(중요)**: 원 출처의 "3분 이상"은 **탐지에서 대응까지의 작전반응시간(end-to-end)**이다. 현행 모델은 이를 **링크 1단계의 지연**으로 적용하여, As-Is 경로 합산 시 `보고 180 + 협조 180 + 교전명령 30 = 390초`로 **원 출처 총량의 2배 이상을 계상**하고 있었다(이중 계상). 본 개정은 이를 용도별 경유 지연으로 재배분하고, **경로 총합이 원 출처의 서술 범위(180~300초)에 들어오도록** 캘리브레이션한다.
...
- **값/분포**: 대표 20 s · **Normal(평균 20, σ5)** — ADR-080에서 균등(10,30)을 정규로 교체. ±2σ가 종전 구간(10~30)과 일치하고, 절차 지연은 대표값 부근에 몰리는 것이 자연스럽다. 음수 꼬리는 엔진이 0으로 절단
- **단위**: 초
...
- **값/분포**: 서버 1개, 처리 중 포함 수용 4건, 지연 **Normal(평균 135, σ 22.5) s**(±2σ = 90~180), 신선도 300 s
- **[분포 변경 이력 · ADR-082]** 종전 Triangular(90, 180, 270)(평균 180). 형태는 음성 협조(ADR-080)와 같은 논리로 정규분포화했고, **범위를 90~180으로 좁히면서 대표값이 180 → 135로 25% 내려갔다.** ⚠️ 이 채널만 **유한 용량(1채널·4건)**이라 지연이 곧 서비스율이고 드롭률로 증폭되므로 형태 정리가 아니라 실질 변경이다. 대표값 유지안 `Normal(180, σ45)`와 12 seed 쌍체로 대조한 결과 **통계적으로 구분되지 않아**(Δ격추 38.7 [34.2, 43.2] vs 38.1 [33.6, 42.5]) 사전 등록한 선택 규칙에 따라 이쪽을 택했다. ⚠️ Triangular → Normal은 난수 소비가 1회 → 2회(Box-Muller·스페어 캐시)로 바뀌어 **모든 지문이 재배열된다** — 단일 seed 값을 방향 근거로 인용하면 안 된다.
- **단위**: 메시지 건·초
...
### [C2-CHAT-TRACK-01] MCRC→군단 AOC 공중항적 중계 (문자·서버 채팅)
- **값/분포**: 대표 45 s, **Normal(평균 45, σ 7.5) s**(±2σ = 30~60)
- **[분포 변경 이력 · ADR-082]** 종전 Triangular(20, 45, 120). 최빈값은 45로 같지만 **평균이 61.7 → 45 s로 27% 내려갔다** — 종전 삼각분포가 오른쪽으로 길게 치우쳐 있었기 때문이다(평균 = (20+45+120)/3). 즉 이것도 형태 정리가 아니라 실질적인 속도 향상이며, **긴 꼬리(최대 120 s)를 잘라내는 것이 실제 절차를 더 잘 표현하는지에 대한 근거는 없다**(등급 C 유지).
- **단위**: 초
- **출처**: As-Is 육군 군단·수방사 AOC와 공군 MCRC 사이 교신 수단은 **음성과 문자(서버 채팅)**라는 실무 확인. 종전에는 이 중계를 데이터링크 1 s로 두어 육↔공 실시간 항적 데이터링크가 있다는 전제가 됐는데, 사람이 채팅창에 적어 넘기므로 그 전제가 성립하지 않는다.
- **적용범위**: As-Is 육↔공 항적 문자 채널 **양방향** — 하행 `MCRC→ARMY_LOCAL_AD`(`mcrc_to_corps_aoc_track`) + 상행 `ARMY_LOCAL_AD→MCRC`(`corps_aoc_track_share`, ADR-080 신설 — 방공C2A가 융합한 국지항적을 MCRC로 전파. 국지레이더의 MCRC 직보를 걷어낸 대체 경로다). To-Be에는 두 계선 모두 없다 — 조율층(IAOC)이 배포한다(ADR-078).
- **신뢰도 등급**: C(정책연구용 개념값 — 근거 문헌 없음). 음성 협조 20 s(C2-VOICE-COORD-01)와 음성/VTC 180 s(C2-ENG-STATUS-01) 사이에 놓았다. 타이핑·확인 왕복이 있어 음성 협조보다 느리고, 비동기라 VTC보다는 빠르다는 판단.
- **MC 적용방식**: 전달지연 삼각분포 샘플링. 채널 용량·드롭은 두지 않는다(항적 중계는 `status`와 달리 유한 메시지 채널로 모델링하지 않음).
- **비고**: ⚠️ **결론 민감도 실측** — SC3 ×1.5 · 12 seed 쌍체 비교에서 이 값은 결론을 바꾸지 않는다. As-Is 격추 평균 63.8±2.6(1 s) → 65.3±2.4(45 s) → 59.8±1.9(180 s), 쌍체 Δ(To-Be−As-Is) 37.7[32.8,42.5] → 36.2[31.3,41.0] → 41.6[37.3,45.9]. **45 s와 종전 1 s는 통계적으로 구분되지 않으며 Δ는 오히려 미세하게 줄어든다** — 이 수정은 To-Be에 유리하게 작용하지 않는다. 180 s처럼 극단으로 가야 유의한 차이가 난다. ⚠️ 단일 seed로는 잡음이 추세를 덮으므로(1/20/45/90/180 s에서 격추 64/59/73/72/64로 비단조) 이 값을 단일 실행으로 판정하면 안 된다. ADR-079.
...
### [C2-KVMF-LATERAL-01] 방공C2A 간 상급 경유 항적 공유 (kvmfLateral, ADR-081)
- **값/분포**: 대표 30 s (고정). 분포 없음.
- **단위**: 초
- **출처(확인된 것)**: KVMF는 육군 지상전술데이터링크 **표준(MND-STD-0016, 2012)** 이고 방공C2A에 실제 적용된다. 방공C2A 전력화로 **작전 반응시간 3분+ → 30초**가 공표됐다(방위사업청·한화시스템 2019 초도양산; 뉴시스·정책브리핑). KVMF는 준실시간으로 규정된다.
- **출처(확인 못 한 것)**: **두 방공C2A 간 직접 peer 연동의 근거는 찾지 못했다.** 방공C2A는 군단/사단 지역을 묶고, 상급 정보는 지상전술C4I(ATCIS)·위성 전군방공경보 계통에서 받는 구조다. 그래서 peer 직결이 아니라 **상급 경유 계선**으로 둔다.
- **적용범위**: `ARMY_LOCAL_AD ↔ ARMY_LOCAL_AD` `report` 링크(`corps_aoc_kvmf_lateral`), **양 모드**. 플래그 `kvmfLateral`(기본 ON, `features.kvmfLateral:false`로 해제 — UI 토글·딥링크 없음. `commandLifecycle`·`commandReceptionQueue`와 같은 등급의 내부 플래그다).
- **신뢰도 등급**: **C**
- **MC 적용방식**: 고정
- **⚠️ 한계 ① — 성격이 다른 값을 옮겨 썼다**: 30초는 **링크 1단계의 전송 지연이 아니라 체계 전체의 작전 반응시간**이다(탐지→대응 end-to-end). `C2-RESP-E2E-01`이 "3분 이상"을 경로 총합 상한으로만 쓰라고 못 박은 것과 같은 종류의 값을, 여기서는 **한 간선의 지연으로** 쓰고 있다. 이 계선의 지연을 "전선 성능"으로 읽으면 안 되고, 다른 링크 파라미터와 나란히 비교해서도 안 된다.
- **⚠️ 한계 ② — 상급 노드를 두지 않고 왕복을 축약했다**: 중간에 지상전술C4I 노드를 두면 그 큐 파라미터(서버 수·서비스시간)를 지어내야 하는데 근거가 없다. 엔진에서 경유 노드는 어차피 큐를 갖지 않으므로(ICC 실측 — 정상 배치 SC3 900초에서 링크 통과 488건 · 큐 도착 0건) 축약해도 동역학은 같다. 대가는 **그림에서 peer 직결로 보인다**는 것이며, [C2 구조] 탭 독해 시 이 축약을 함께 밝혀야 한다.
- **⚠️ 한계 ③ — 임무 지표를 움직이지 않는다(실측)**: SC3 ×1.0 · 900초 · **30 seed As-Is 자기쌍체(OFF↔ON)** 에서 Δ격추 −0.57 [−2.57, +1.44] · Δ누수 +0.57 [−1.38, +2.51] · Δ중복교전 −0.13 [−0.80, +0.54] 등 **8개 지표 전부 CI가 0을 포함**한다. 이유는 기하다: 국지방공 교전반경(비호 7 km)이 두 권역 간 거리(SHORAD_1C↔SHORAD_CD 20.4 km)보다 작아 **상대 권역 표적에 사거리가 닿지 않는다** — 정보는 90초에서 30초로 빨라지지만 교전 기회는 늘지 않는다. 중첩이 있는 것은 천궁-II(MSAM_1C, 50 km) 한 문뿐이고 그 한 문은 이미 자기 권역 레이더로 그 공역을 본다.
- **비고**: 채택 근거는 **수치 개선이 아니라 구조 정합**이다 — 2022-12-26 사건의 공식 지적(1군단↔수도방위사령부 항적 공유 지연)이 걸리는 계선이 모델에 한 가닥도 없어, 두 방공C2A가 서로의 그림을 보려면 공군 MCRC를 문자로 90초 왕복해야 했다(육군 자체 망이 모델에 없었다). ADR-081.
...
- **MC 적용방식**: 고정. ~~1단계 근사(센서별 고정 지연 = reportingPeriod)~~ → **ADR-069로 톱니 신선도가 구현되어 이 값은 이제 "주기 P" 자체이며 소비되는 지연은 IADS-LINK-SAW-01이 계산한다.**

### [IADS-LINK-SAW-01] 보고 주기 톱니 신선도 (sawtoothFreshness, ADR-069)
- **값/분포**: 파생값 — `staleness(P, t) = t − ⌊t/P⌋·P`, 치역 `[0, P)`, 시간평균 `P/2`. 주기 P는 IADS-LINK-RP-01.
- **단위**: 초
- **출처**: `IADS_codex/src/engine/*` — codex ADR-014 정본: "탐지자산→상위 C2 정보 전달은 일률 지연이 아니라 보고 주기 — **그림 나이가 0~P 톱니(sawtooth)로 변동**". Air_Defense는 ADR-057에서 주기 P를 상수 지연으로 소비하는 1단계 근사만 이식했고, 톱니 자체는 ADR-069에서 이식했다.
- **적용범위**: `comm.type === 'report-cycle'`인 링크에만 적용 — 즉 IADS-LINK-RP-01이 지배하는 센서→C2 `report` 구간. **음성 협조 링크(C2-COORD-HORIZ-01, 4개)와 C2↔C2 전송(IADS-LINK-SHORT-01/IFCN-01)은 제외** — 이들은 주기 갱신이 아니라 사건 기반 전송이므로 톱니 개념이 성립하지 않는다. 플래그 `sawtoothFreshness`(ADR-072로 **기본 ON**, `saw=0`으로 해제).
- **⚠️ 전역 동기 시계 가정**: codex와 동일하게 모든 센서의 보고 시점이 `t = 0`을 공유하는 전역 동기 시계를 가정한다. 실제로는 센서별 위상(phase)이 무작위이지만, codex 정본이 그렇고 위상 난수를 도입하면 정본과 bit 비교가 불가능해진다. 개념적 한계로 ADR-069 §한계에 기록.
- **신뢰도 등급**: **C** — 톱니 형태 자체는 주기 갱신 센서의 물리적 성질이므로 구조 근거는 있으나, 주기 P가 이미 등급 C(codex 스스로 placeholder라 명시)이므로 파생값도 C를 넘지 못한다.
- **MC 적용방식**: 결정적 — 난수 없이 시뮬레이션 시각 `t`의 함수. 도착 시각이 MC 반복마다 달라지므로 신선도 분포는 반복 간에 자연히 변동한다(별도 RNG 스트림 불필요).

### [IADS-SDF-R-01] 자위권 사격 반경 (selfDefenseFire, ADR-071)
- **값/분포**: 10 km (고정)
- **단위**: km
- **출처**: `IADS_codex` ADR-050(실패 분류 v2) 3단 발사 사다리 ③ — 상급 화력통제·원격 화력통제가 모두 성립하지 않아도 **포대 자신에게 직접 임박한 위협**은 자위권 차원에서 사격한다. codex의 자위권 판정 반경 승계.
- **적용범위**: 탄도탄 위협에만 적용(항공기·무인기는 제외 — 자위권 판단의 즉시성 근거가 탄도탄 종말단계 임박성에 있고, 저속 위협은 상급 협조 시간이 확보된다). 포대 자기 센서가 탐지하고 예측 탄착점이 포대 10 km 이내이며 상급/원격 화력통제가 없을 때만 발동. 플래그 `selfDefenseFire`(ADR-072로 **기본 ON**, `sdf=0`으로 해제).
- **신뢰도 등급**: **C** — 개념 설정. 자위권 사격 자체는 실제 교전규칙에 존재하는 개념이나, 10 km라는 임계값은 codex 개념값이며 공개근거 없음.
- **MC 적용방식**: 고정

### [IADS-EOR-01] 원격 화력통제 교전 (engageOnRemote, ADR-070)
- **값/분포**: 불리언 — 등급 판정만, 수치 파라미터 없음
- **단위**: 없음
- **출처**: `IADS_codex` ADR-051 D2 — 킬웹 내 다른 포대/센서가 확보한 **화력통제 품질 항적**으로 사격할 수 있다(engage-on-remote). 단 **탐지 전용 자산(조기경보 레이더)은 제외** — 탐지 등급 항적은 사격 해를 만들 수 없다.
- **적용범위**: `_iadsRemoteFcGrade()`가 ADR-051 D1 웹 파티션(`upper` / `local_ad` / `usfk`) 경계 안에서만 원격 항적을 허용한다. 웹을 넘는 원격 화력통제는 성립하지 않는다(국지방공 포대가 상급 킬웹 항적으로 쏘지 못하고, USFK 축은 ADR-036 독립성 유지).
...
- **비고 (Phase 4 용량 감사, 실측 seed 1~10 평균)**:
  ...
  - ⚠️ **취약점**: To-Be의 **누수율** 개선폭이 이 스펙에 상당히 민감하다. 허브를 As-Is 수준(c=3·25s)으로 낮추면 SC3 x2.5 누수율 47.9%→72.7%로 As-Is(73.0%)에 수렴, x3.0에서는 역전. 반면 **결심지연·격추율 개선은 스펙 무관하게 견고**(구조 효과). 상세·판단은 `구 vv-report(문서 폐기 — 모의논리서 §6으로 대체)` §4-3.
...
- **값/분포**: As-Is `VOICE`(대표 20 s · Normal(20,σ5) — `C2-VOICE-COORD-01` 승계) / To-Be `DL_FAST` 2 s (linkSemanticsV2 ON이면 IFCN 1 s)
- **단위**: 초
...
- **적용범위**: `approvalChain` ON일 때 native `_iadsDecide`의 As-Is `LOCAL_AD` 축(ROK 국지방공)에만 단계+처리. 타 한국군 축은 승인권자=자기 자신(거치지 않음), **USFK 축은 ADR-036에 따라 계선 미적용**(어서션 고정). `approvalChainTobe`는 반증 전용(To-Be에 As-Is 계선 강제)
- **신뢰도 등급**: 해당 없음(승계 명세)
- **MC 적용방식**: 승계 파라미터의 방식을 따름

### [C2-COORD-HORIZ-01] 수평 교전협조 링크 (Phase 2 ⑥⑦, `js/data/links.js`)
- **값/분포**: As-Is 신규 coord 링크 — `AOC-1C↔JAOC-CD`(양방향)·`MCRC→AOC-1C`·`MCRC→JAOC-CD`. As-Is=`VOICE`(**Normal 평균20/σ5**, 대표 20s — 2026-07-24 실험으로 삼각(90/180/270)→균등(10,30), ADR-080에서 균등→정규), To-Be=`DL_FAST`(2s)
- **단위**: 초(링크 전달 지연)
...
- **값/분포**: human-in-loop(승인권자까지 협조경로+승인 처리) / human-on-loop(감독하 자동교전 — 승인 처리만, 협조 단계 생략) / auto-preauth(사전승인 자동교전 — 결심 경유 생략). As-Is 전 위협 in-loop; To-Be 무인기·순항(주1)·탄도탄 auto-preauth 또는 on-loop, 유인기 위협 on-loop
- **출처**: 합참 K-JAMDS 구축 개념안(AI 기반 표적 식별·무기 배정)·무인기 사전승인 자동교전 개념 — 기존 threats.js note 텍스트의 엔진 플래그 승격
- **적용범위**: DES `_decision` (구 approval=null 우회의 일반화). 주1: cruise는 on-loop이나 To-Be approvalLevel=null이라 경유 없는 감독 자동교전으로 동작
- **신뢰도 등급**: C(개념 구분)
- **MC 적용방식**: 고정
- **비고 (자기 승인 — ⑥⑦ 병목의 실제 대상, Phase 3 감사)**: `_decision`은 **담당 C2와 승인권자(`approvalLevel[mode]`)가 동일하면 승인 단계를 생략**한다(`approvalId === controlC2`). As-Is에서 **순항미사일(담당 MCRC=승인 MCRC)·탄도탄(담당 KAMDOC=승인 KAMDOC)은 자기 계통 내 자기 승인**이라 육↔공 이원화 병목을 겪지 않는다. 실측(SC3 x1.5 As-Is): 전체 결심의 **약 49%가 승인 단계 생략**(순항·탄도탄 자기승인). **의도된 모델링** — 이원화 마찰은 육군↔공군(및 최상위 KAOC 승인) 사이에서 발생하며, 공군 방공(MCRC)·대탄도탄(KAMDOC) 계통은 각자 담당 위협을 자기 계통 안에서 승인한다. 따라서 ⑥⑦ "핵심 병목"은 **육↔공 경유 위협(무인기·저속기·헬기·전투기)에 한정**해 정확하다(상세: `docs/분석가_가이드.md`). **approvalLevel 데이터는 이 감사에서 변경하지 않았다.**
...
### [C2-AUDIT-CAND-CAP-01] 결심 감사 위협당 후보 상한 (ADR-073, `js/engine/sim-engine.js`)
- **값/분포**: 30 (features.decisionAuditMaxCandidates로 재정의 가능)
- **단위**: 후보 수/결심
- **출처**: **공개근거 없음** — 이벤트 크기 관리용 공학적 상한이며 작전 개념이 아님
- **적용범위**: `_emitDecisionAudit` — `decision_audit.candidates` 배열 길이. 배열은 이미
  점수 내림차순으로 정렬돼 있으므로 상위 N 절단은 결정론이며, 실제 선택은 언제나 rank 0이라
  절대 잘리지 않는다. 잘린 결심은 `candidatesTruncated=true`, 실제 후보 수는 `candidateCount`로 보존.
- **신뢰도 등급**: C(공개근거 없음 — 계측 구성값)
- **MC 적용방식**: 고정. 계측 전용이라 물리·결심·RNG에 무관(OFF/ON bit-exact·RNG 불변 실측).
- **비고**: 실측(seed 12345×900초, SC1/SC3)에서 `candidateCount` 최대 4로 상한이 걸리지 않았다.

### [C2-AUDIT-EVT-CAP-01] 결심 감사 런당 이벤트 상한 (ADR-073)
- **값/분포**: 5000 (features.decisionAuditMaxEvents로 재정의 가능)
- **단위**: 이벤트 수/런
- **출처**: **공개근거 없음** — 메모리 백스톱
- **적용범위**: 초과 시 이후 결심을 기록하지 않고 `global.decisionAudit.{dropped,truncated}`로 공시
- **신뢰도 등급**: C(공개근거 없음 — 계측 구성값)
- **MC 적용방식**: 고정
- **비고**: ⚠️ **후반부 시간 편향이 있다**(초과 시점 이후를 버림). 편향 없이 로그를 줄이려면
  아래 표본추출률을 쓸 것. 실측 결심 수는 케이스당 27~154건으로 상한의 1/30 이하다.

### [C2-AUDIT-SAMPLE-01] 결심 감사 결정론 표본추출률 (ADR-073)
- **값/분포**: 1.0 (전수). 0<r≤1 범위에서 features.decisionAuditSampleRate로 재정의
- **단위**: 비율
- **출처**: **공개근거 없음** — 관측 표본 설계
- **적용범위**: threatId의 FNV-1a 해시를 [0,1)로 정규화해 r 미만인 **위협만** 기록.
  결심 시각·결과와 무관한 위협 단위 채택이라 시간·결과 무편향이며, CRN으로 As-Is/To-Be가
  같은 위협집합을 마주하므로 **두 모드가 같은 표본을 본다**(짝지은 비교 보존).
- **신뢰도 등급**: C(공개근거 없음 — 계측 구성값)
- **MC 적용방식**: 고정(결정론 — 동일 seed 재실행에서 동일 표본, 테스트로 고정)
- **비고**: 표본 적용 시 `global.decisionAudit.sampledOut`에 제외 건수가 남는다.

### [C2-SHADOW-SCOPE-01] 그림자 평가 자산 범위 (ADR-074, `js/engine/sim-engine.js` `_shadowShooters`)
- **값/분포**: 한국군 전 발사대(`forceOwner` ∈ {ROK, ROK_LOCAL_AD}). **USFK 기본 제외.**
  `features.shadowEvalIncludeUsfk=true`로 포함 반사실을 참고용으로만 켤 수 있다.
- **단위**: 자산 집합
- **출처**: ADR-036(USFK 권한 자동 통합 금지) 승계 — USFK는 독립 C2/WTA 축이므로
  "통합됐다면 골랐을 최적"이라는 반사실에 넣으면 독립성 가정 자체를 훼손한다.
- **적용범위**: `regret`의 분모가 되는 전역 최적 사수 탐색. USFK 축이 내린 결심은 `regret`을
  0이 아니라 **null(미측정)**로 남기고 `shadowScope='skipped_usfk_axis'`로 공시한다.
  ⚠️ C2 배정 제약(`coverage` 담당 축)은 **의도적으로 무시**한다 — 그림자 평가가 완화하려는
  것이 바로 그 제약이다. 물리 능력·실현가능성(봉투·PIP·FC·탄약·채널)은 그대로 적용된다.
- **신뢰도 등급**: B(기존 ADR-036 결정 승계)
- **MC 적용방식**: 고정(결정론 — 난수 소비 0, 테스트로 고정)
- **비고**: 실제 선택도 이 집합의 원소이므로 `regret ≥ 0`이 구조적으로 보장된다.

### [C2-WINDOW-CLOSE-01] 교전창 마감 정의 (ADR-074, `_engagementWindowOf`)
- **값/분포**: 마감 = 전 자산 반사실 PIP 창(`_iadsGeometryWindow`) `lastFire`의 **최댓값**,
  개시 = `firstFire`의 **최솟값**. 어떤 자산도 창을 못 잡으면 null(미측정).
- **단위**: 초(시뮬레이션 시각)
- **출처**: **새 물리 없음** — `_terminalIadsFailure`가 이미 쓰던 `latestFire`와 동일 재료의
  상·하한 재사용. 하부 창 자체의 근거는 [WPN-*-ENV-01] 교전 포락선·유도탄 속도.
- **적용범위**: `window_audit`(위협 전수) 및 `decision_audit.engagementWindowMargin`
  (= 마감 − 결심완료). 창은 궤적·자산 배치만의 함수라 위협 생성 시점에 확정된다.
- **신뢰도 등급**: B(기존 파라미터의 파생 — 새 수치를 도입하지 않음)
- **MC 적용방식**: 고정(결정론·캐시)
- **비고**: ⚠️ **기하학적 상한**이다(탄약·채널·FC를 정상화한 반사실). 실제 자원 상태를 반영한
  창이 아니다. 또한 결심 시점 여유는 생존 편향이 있어(결심은 실현가능 PIP가 있어야 성립하므로
  거의 음수가 안 된다) **놓침률의 분모는 반드시 `window_audit`(창이 있었던 전 위협)**으로
  잡고 미결심 위협을 놓침으로 세야 한다. 상세는 ADR-074 §B-2.
...
- **신뢰도 등급**: C
- **MC 적용방식**: 분포샘플링
- **비고**: 주간/야간 구분은 이번 범위 밖 — 단일 혼합값(0.35)으로 처리(별도 이슈, 구 vv-report(문서 폐기 — 모의논리서 §6으로 대체) 잔여 한계 참조).
...
- **비고**: ⚠️ 전체 bit-exact 기준선(0468f10) 복원은 CRN(arrRng 분리)이 도착 스트림을 재배치해 **불가**. 되돌리기는 ⑨ 런타임 플래그(36/36)+이 플래그+git 층위로 제공. 상세 당시 통합 감사 G2(문서 폐기 — ADR 원장으로 대체)
...
- **신뢰도 등급**: C(시각화 목적 개념값)
- **MC 적용방식**: 고정
- **비고**: ADR-064로 남부 종심 축선 2종 추가 — `southcentral`(평강 개념→대구, 306km)·`southeast`(평강 개념→부산, 400km). 두 축선은 `features.southernAxes` ON일 때만 coverage 파생·도착 예약에 참여한다(OFF는 종전 4축선 wire shape). 시나리오 축선 키와 1:1 대응. 구 좌표(백령도·철원·고성·고양 개념)는 위협 다양화 개편에서 북측 개념좌표로 대체

### [THREAT-TARGET-DISP-01] 표적권역 착탄점 산포 반경 (ADR-063)
- **값/분포**: 반경 **15 km 원판(disk) 내 균등(면적 기준)** — 착탄점 = 축선 표적점 + (r, θ), r = R·√u₁, θ = 2π·u₂. 이론 평균 이격 2R/3 = 10 km (실측 10.1 km로 일치 확인)
- **출처**: **개념 설정 — 공개근거 없음(등급 C)**. 축선 표적의 성격(`targetNote`: "서울 개념좌표(방호 표적권역)", "오산·평택 권역", "동해 함대 권역")이 점이 아니라 **도시·권역**이라는 점에서 도시권 규모의 개념 반경을 부여한 것이며, 실제 표적·조준정확도·CEP가 아니다
- **적용범위**: `features.threatTargetDispersion` ON일 때 위협 생성 시 1회 추첨(전용 RNG 스트림 `dispRng`). OFF면 추첨하지 않아 종전과 bit-exact. `features.targetSpreadKm` 숫자로 재정의(스윕) 가능
- **신뢰도 등급**: C(개념 설정 — 공개근거 없음)
- **MC 적용방식**: 복제마다 위협별 독립 추첨(도착·처리 스트림과 분리 — 산포를 켜도 다른 도메인의 난수 소비 순서 불변)
- **비고**: 반경 선택의 제약 — 축선 표적 간 최소 거리는 서울↔오산·평택 약 44 km이므로, R=15 km는 **다른 축선 표적권역을 침범하지 않는다**(권역 혼입 방지). 산포는 축선 개념거리를 최대 R만큼 늘리므로 사거리 정합(ENV-AXIS-FIT-01)을 `conceptReachKm + R` 기준으로 재검증했다(전 시나리오 mix 위반 0건). ⚠️ **산포는 표적권역 "내부"를 다양화할 뿐 새 표적권역을 만들지 않는다** — 부산·대구 등 남부 표적은 축선 추가(별도 결정 사안)가 있어야 발생한다

### [THREAT-SOUTH-SHARE-01] 남부 종심 축선 도착률 배분비 (ADR-064)
- **값/분포**: 남부 축선 λ = **수도권 대응 축선 λ × 0.1** — SC3 기준 srbm 1.5→0.15(대구·부산 각), mrl_large(central) 1.0→0.10, cruise 0.3→0.03, fighter(central) 0.3→0.03
- **출처**: **개념 설정 — 공개근거 없음(등급 C)**. 개별 λ를 7개 지어내지 않고 **배분비 1개**만 정의해 파생한다(검토 가능성 우선). 종심 타격이 수도권 축선보다 드물다는 개념 가정
- **적용범위**: `features.southernAxes` ON일 때 SC3의 `southernMix` 도착 예약. SC1·SC2는 수도권 근접 상황이라 미적용
- **신뢰도 등급**: C(개념 설정 — 공개근거 없음)
- **MC 적용방식**: 포아송 도착(전용 스트림 `southRng` — 기존 축선 도착 스케줄 불변)
- **비고**: 배정 가능 위협은 사거리 정합(ENV-AXIS-FIT-01)이 결정한다 — 무인기(최대 300km)·헬기는 **개념 사거리 미달로 자동 거부**되고, 방사포(최대 400km)는 부산축(400km + 산포 15km = 415km)에서 초과라 **대구축에만** 배정된다

### [THREAT-AXIS-DWELL-SCALE-01] 남부 축선 체공시간 거리 환산 (ADR-064)
- **값/분포**: `dwell_남부 = dwell_기본 × (축선 개념거리 / 140km)` — 기준거리 140km는 중부축(central) 개념거리. 실효값: srbm 90→197s(대구)·257s(부산), 방사포 80→175s, 순항 120→262s·343s, 전투기 180→393s·514s
- **출처**: **개념 설정 — 공개근거 없음(등급 C)**. 위협 위치가 `lerp(entry, target, (t−spawnT)/dwellSec)`이라 **체공시간을 그대로 두면 먼 축선일수록 함의 속도가 비례해 빨라진다**(순항미사일 120초에 400km = 3.3km/s, 자기 baseSpeed 272m/s의 12배). 새 속도 값을 만들지 않고 **모델이 이미 쓰던 중부축 함의 속도를 그대로 유지**하도록 체공시간만 거리 비례로 늘린다
- **적용범위**: 남부 축선(`SOUTHERN_AXIS_KEYS`)에만 적용 — 기존 4개 축선의 dwellSec은 불변(bit-exact 보존)
- **신뢰도 등급**: C(개념 설정 — 공개근거 없음)
- **MC 적용방식**: 고정(축선·위협유형의 결정론적 함수)
- **비고**: ⚠️ 이 환산은 **기존 모델의 함의 속도를 보존**할 뿐, 그 속도 자체가 실제 위협 속도와 일치한다는 뜻이 **아니다** — 함의 속도(중부축 순항 1.17km/s)는 `THREAT_PHYSICS.baseSpeed`(272m/s)와 여전히 어긋난다. 이 불일치는 개념 축선/dwell 모델의 기존 한계이며(모의논리서 §7), 본 ADR의 범위 밖이다
...
- **비고**: legacy 10세트 확장 후 대표 결과(SC3, seed=12345): ρ≥0.9 돌파 ×1.5,
  ...
  배치·절단시점의 결과이며 코드에서 강제하지 않음(`구 vv-report(문서 폐기 — 모의논리서 §6으로 대체)` §3.4).
